...
function isMobileComposerLayout() {
    ...
    return mobileUserAgent || ipadDesktopMode || (coarsePointer && maxTouchPoints > 0 && Math.min(window.innerWidth, window.innerHeight) <= 1024);
}

function syncScreenMetrics() {
    const screenHeight = Math.max(
        Math.round(window.screen?.height || 0),
        Math.round(window.screen?.availHeight || 0),
        Math.round(window.innerHeight || 0)
    );
    chatPage.style.setProperty("--screen-height", `${screenHeight}px`);
}

function shouldCollapseComposerTray() {
    return document.activeElement === inputBox && isMobileComposerLayout();
}
...
function syncViewportLayout({ preserveScroll = true } = {}) {
    ...
    viewportState.frame = requestAnimationFrame(() => {
        ...
        const shouldCollapseTray = shouldCollapseComposerTray();
        const appliedKeyboardInset = shouldCollapseTray ? keyboardInset : 0;
        ...
        chatPage.style.setProperty("--keyboard-inset", `${appliedKeyboardInset}px`);
        ...
        chatPage.classList.toggle("keyboard-open", shouldCollapseTray);
        viewportState.lastKnownKeyboardInset = appliedKeyboardInset;
